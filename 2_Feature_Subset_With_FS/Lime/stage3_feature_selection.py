...
# List all LIME files to display options to the user
datasets = [f for f in os.listdir(score_dir) if f.endswith('_LIME_Scores.csv') and not f.startswith('._')]
# datasets = [f for f in os.listdir(score_dir)]
dataset_names = [os.path.splitext(f)[0].replace('_LIME_Scores', '') for f in datasets]
...
for idx in selected_indices:
    ...
    # Sort the features by LIME in descending order
    sorted_feature_scores = feature_scores.sort_values(by='LIME_Score', ascending=False)
    ...
    if choice == 1:
        ...
        selected_features = sorted_feature_scores[sorted_feature_scores['LIME_Score'] >= threshold]['Feature']
    ...
    with open(report_file, 'w') as f:
        f.write("LIME Feature Selection Report\n")
        f.write("=====================================\n")
        ...
        f.write(f"LIMEs CSV File: {score_file}\n")
