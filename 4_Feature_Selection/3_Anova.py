...
# Base directory
BASE_DIR = "AttBiLSTM_Analysis_With_ANOVA_FS"
...
# Initial split for ANOVA feature selection
...
def perform_anova_feature_selection(X_train, y_train, X_val, y_val,
                                    feature_names, n_features=2000):
    """
    Perform ANOVA F-test based feature selection
    Select top n_features COLUMNS (features) from the dataset using ANOVA F-statistic
    """
    print("=" * 60)
    print("PERFORMING ANOVA FEATURE SELECTION")
    print("=" * 60)
    ...
    # Step 1: Perform ANOVA F-test for feature selection
    print("Computing ANOVA F-statistics for all features...")
    ...
    # Create feature importance based on F-scores
    feature_importance = f_scores.copy()

    # Print some statistics
    selected_f_scores = f_scores[selected_indices]
    selected_p_values = p_values[selected_indices]

    print(f"F-score statistics for selected features:")
    print(f"  Mean F-score: {np.mean(selected_f_scores):.4f}")
    print(f"  Std F-score: {np.std(selected_f_scores):.4f}")
    print(f"  Min F-score: {np.min(selected_f_scores):.4f}")
    print(f"  Max F-score: {np.max(selected_f_scores):.4f}")

    print(f"P-value statistics for selected features:")
    print(f"  Mean p-value: {np.mean(selected_p_values):.6f}")
    print(f"  Max p-value: {np.max(selected_p_values):.6f}")
    print(f"  Features with p < 0.05: {np.sum(selected_p_values < 0.05)}")
    print(f"  Features with p < 0.01: {np.sum(selected_p_values < 0.01)}")
    ...
    print(f"Logistic Regression on ANOVA selected features:")
    print(f"  Train Accuracy: {train_score:.4f}")
    print(f"  Validation Accuracy: {val_score:.4f}")

    # Step 3: Save ANOVA analysis results
    anova_results = {
        ...
        'features_p_less_0_05': int(np.sum(selected_p_values < 0.05)),
        'features_p_less_0_01': int(np.sum(selected_p_values < 0.01))
    }

    # Save ANOVA results
    anova_results_path = os.path.join(BASE_DIR, 'feature_selection', 'anova_results.json')
    with open(anova_results_path, 'w') as f:
        json.dump(anova_results, f, indent=2)

    # Save the ANOVA selector
    anova_selector_path = os.path.join(BASE_DIR, 'feature_selection', 'anova_selector.joblib')
    joblib.dump(selector, anova_selector_path)

    # Create feature importance plot
    plot_anova_feature_importance(feature_importance, feature_names, selected_indices, n_features, p_values)

    print(f"ANOVA analysis results saved to: {anova_results_path}")
    print(f"ANOVA selector saved to: {anova_selector_path}")

    return selected_indices, selected_features, feature_importance



def plot_anova_feature_importance(feature_importance, feature_names, selected_indices, n_features, p_values):
    """Plot ANOVA feature importance (F-scores)"""
    ...
    # Create horizontal bar plot with color coding based on p-values
    y_pos = np.arange(len(top_names))
    colors = ['darkgreen' if p < 0.01 else 'green' if p < 0.05 else 'orange' for p in top_p_values]
    bars = plt.barh(y_pos, top_importance, color=colors, edgecolor='black', linewidth=0.5)

    plt.yticks(y_pos, top_names, fontsize=8)
    plt.xlabel('ANOVA F-Score', fontsize=14)
    plt.ylabel('Features', fontsize=14)
    plt.title(f'Top {len(top_names)} Features by ANOVA F-Score', fontsize=16, pad=20)
    ...
    legend_elements = [
        ...
        Patch(facecolor='green', label='0.01 ≤ p < 0.05'),
    ...
    # Save plot
    importance_plot_path = os.path.join(BASE_DIR, 'feature_selection', 'anova_feature_importance.png')
    plt.savefig(importance_plot_path, dpi=300, bbox_inches='tight')
    plt.savefig(importance_plot_path.replace('.png', '.pdf'), dpi=300, bbox_inches='tight')
    plt.close()

    # Plot distribution of all F-scores
    plt.figure(figsize=(12, 4))

    # Subplot 1: F-score distribution
    plt.subplot(1, 2, 1)
    plt.hist(feature_importance, bins=50, color='skyblue', edgecolor='navy', alpha=0.7)
    ...
    plt.subplot(1, 2, 2)
    plt.hist(-np.log10(p_values + 1e-10), bins=50, color='lightcoral', edgecolor='darkred', alpha=0.7)
    plt.axvline(-np.log10(0.05), color='red', linestyle='--', linewidth=2, label='p = 0.05')
    plt.axvline(-np.log10(0.01), color='darkred', linestyle='--', linewidth=2, label='p = 0.01')
    plt.xlabel('-log10(p-value)', fontsize=12)
    plt.ylabel('Number of Features', fontsize=12)
    plt.title('Distribution of -log10(p-values)', fontsize=14)
    plt.legend()
    plt.grid(alpha=0.3)

    plt.tight_layout()

    dist_plot_path = os.path.join(BASE_DIR, 'feature_selection', 'anova_distributions.png')
    plt.savefig(dist_plot_path, dpi=300, bbox_inches='tight')
    plt.savefig(dist_plot_path.replace('.png', '.pdf'), dpi=300, bbox_inches='tight')
    plt.close()

    print(f"ANOVA importance plots saved to: {importance_plot_path}")
    print(f"ANOVA distribution plots saved to: {dist_plot_path}")



# Perform ANOVA Feature Selection
print("\n" + "=" * 60)
print("STARTING ANOVA FEATURE SELECTION")
print("=" * 60)

selected_indices, selected_features, feature_importance_scores = perform_anova_feature_selection(
    X_train_initial, y_train_initial, X_val_initial, y_val_initial,
    feature_names, n_features=2000
)

# Apply feature selection to all datasets
print("\nApplying ANOVA feature selection to datasets...")
...
def plot_confusion_matrix(cm, class_names, save_path):
    ...
    plt.title('Confusion Matrix of AttBiLSTM with ANOVA FS', fontsize=28, pad=20)
...
def plot_training_curves(history, save_path):
    ...
    ax1.set_title('Training and Validation Loss of AttBiLSTM with ANOVA FS', fontsize=28, pad=20)
    ...
    ax2.set_title('Training and Validation Accuracy of AttBiLSTM with ANOVA FS', fontsize=28, pad=20)
...
def plot_training_loss(history, save_path):
    ...
    plt.title('Training and Validation Loss of AttBiLSTM with ANOVA FS', fontsize=28, pad=20)
...
def plot_training_accuracy(history, save_path):
    ...
    plt.title('Training and Validation Accuracy of AttBiLSTM with ANOVA FS', fontsize=28, pad=20)
    plt.xlabel('Epoch', fontsize=24)
...
def plot_roc_curve(roc_data, num_classes, save_path):
    ...
    plt.title('ROC Curves - AttBiLSTM with ANOVA FS', fontsize=28, pad=20)
...
def save_model_architecture(model, file_path):
    ...
    with open(file_path, 'w') as f:
        f.write("Advanced AttBiLSTM Model Architecture with ANOVA Feature Selection\n")
        f.write("=" * 70 + "\n\n")
        ...
        f.write(f"Feature Selection Method: ANOVA F-test\n\n")
...
def run_complete_analysis():
    """Run the complete AttBiLSTM analysis pipeline with ANOVA feature selection"""

    print("=" * 60)
    print("Starting Advanced AttBiLSTM Analysis Pipeline with ANOVA FS")
    print("=" * 60)
    ...
    print(f"Input features: {num_features} (selected by ANOVA from {len(feature_names)} original features)")
    ...
    # Save model
    model_save_path = os.path.join(BASE_DIR, 'models', 'attbilstm_anova_model.pth')
    torch.save({
    ...
    detailed_metrics['Feature Selection Method'] = 'ANOVA'
    ...
    selected_features_df = pd.DataFrame({
        ...
        'anova_f_score': feature_importance_scores[selected_indices]
    })
    ...
    with open(summary_path, 'w') as f:
        f.write("Advanced AttBiLSTM Analysis Summary with ANOVA Feature Selection\n")
        f.write("=" * 65 + "\n\n")
        ...
        f.write(f"Method: ANOVA F-test\n")
        f.write(f"Statistical test: f_classif (univariate F-test)\n")
    ...
    print("\nFiles generated:")
    print("- Models: attbilstm_anova_model.pth, model_architecture.txt")
    print("- Feature Selection: anova_results.json, selected_features.csv, anova_feature_importance.png")
