...
# Base directory
BASE_DIR = "AttBiLSTM_Analysis_With_Lasso_FS"
...
# Initial split for Lasso feature selection
...
def perform_lasso_feature_selection(X_train, y_train, X_val, y_val,
                                    feature_names, n_features=2000):
    """
    Perform Lasso-based feature selection
    Select top n_features COLUMNS (features) from the dataset using Lasso regularization
    """
    print("=" * 60)
    ...
    # Step 1: Train Lasso with Cross-Validation to find optimal alpha
    print("Training Lasso with Cross-Validation to find optimal regularization...")
    ...
    # Step 2: If we have more than target features, use SelectFromModel to get exactly n_features
    if np.sum(lasso_cv.coef_ != 0) > n_features:
        print(f"Lasso selected {np.sum(lasso_cv.coef_ != 0)} features, reducing to {n_features}...")

        # Use feature importance (absolute coefficients) to select top n_features
        feature_importance = np.abs(lasso_cv.coef_)
        selected_indices = np.argsort(feature_importance)[::-1][:n_features]

    elif np.sum(lasso_cv.coef_ != 0) < n_features:
        print(f"Lasso selected only {np.sum(lasso_cv.coef_ != 0)} features, less than target {n_features}")
        print("Using a weaker regularization to get more features...")

        # Use weaker alpha to get more features
        weaker_alpha = lasso_cv.alpha_ * 0.1
        ...
        feature_importance = np.abs(lasso_weak.coef_[0])
        selected_indices = np.argsort(feature_importance)[::-1][:n_features]

    else:
        # Exactly the right number of features
        feature_importance = np.abs(lasso_cv.coef_)
    ...
    print(f"Logistic Regression on selected features:")
    print(f"  Train Accuracy: {train_score:.4f}")
    print(f"  Validation Accuracy: {val_score:.4f}")

    # Step 4: Save Lasso analysis results
    lasso_results = {
        ...
        'non_zero_coefficients': int(np.sum(lasso_cv.coef_ != 0))
    }

    # Save Lasso results
    lasso_results_path = os.path.join(BASE_DIR, 'feature_selection', 'lasso_results.json')
    with open(lasso_results_path, 'w') as f:
        json.dump(lasso_results, f, indent=2)

    # Save the Lasso model
    lasso_model_path = os.path.join(BASE_DIR, 'feature_selection', 'lasso_model.joblib')
    joblib.dump(lasso_cv, lasso_model_path)

    # Create feature importance plot
    plot_lasso_feature_importance(feature_importance, feature_names, selected_indices, n_features)

    print(f"Lasso analysis results saved to: {lasso_results_path}")
...
def plot_lasso_feature_importance(feature_importance, feature_names, selected_indices, n_features):
    """Plot Lasso feature importance (absolute coefficients)"""
    ...
    bars = plt.barh(y_pos, top_importance, color='lightcoral', edgecolor='darkred', linewidth=0.5)

    plt.yticks(y_pos, top_names, fontsize=8)
    plt.xlabel('Absolute Lasso Coefficient', fontsize=14)
    plt.ylabel('Features', fontsize=14)
    plt.title(f'Top {len(top_names)} Features by Lasso Importance', fontsize=16, pad=20)
    ...
    # Save plot
    importance_plot_path = os.path.join(BASE_DIR, 'feature_selection', 'lasso_feature_importance.png')
    ...
    plt.hist(feature_importance, bins=50, color='lightcoral', edgecolor='darkred', alpha=0.7)
    ...
    plt.title('Distribution of Lasso Feature Importance Scores', fontsize=14)
    plt.grid(alpha=0.3)
    plt.tight_layout()

    dist_plot_path = os.path.join(BASE_DIR, 'feature_selection', 'lasso_importance_distribution.png')
    plt.savefig(dist_plot_path, dpi=300, bbox_inches='tight')
    plt.savefig(dist_plot_path.replace('.png', '.pdf'), dpi=300, bbox_inches='tight')
    plt.close()

    print(f"Lasso importance plots saved to: {importance_plot_path}")
    print(f"Lasso distribution plot saved to: {dist_plot_path}")
...
selected_indices, selected_features, feature_importance_scores = perform_lasso_feature_selection(
    X_train_initial, y_train_initial, X_val_initial, y_val_initial,
    feature_names, n_features=2000
)

# Apply feature selection to all datasets
print("\nApplying Lasso feature selection to datasets...")
...
def plot_confusion_matrix(cm, class_names, save_path):
    ...
    plt.title('Confusion Matrix of AttBiLSTM with Lasso FS', fontsize=28, pad=20)
...
def plot_training_curves(history, save_path):
    ...
    ax1.set_title('Training and Validation Loss of AttBiLSTM with Lasso FS', fontsize=28, pad=20)
    ...
    ax2.set_title('Training and Validation Accuracy of AttBiLSTM with Lasso FS', fontsize=28, pad=20)
...
def plot_training_loss(history, save_path):
    ...
    plt.title('Training and Validation Loss of AttBiLSTM with Lasso FS', fontsize=28, pad=20)
...
def plot_training_accuracy(history, save_path):
    ...
    plt.title('Training and Validation Accuracy of AttBiLSTM with Lasso FS', fontsize=28, pad=20)
    plt.xlabel('Epoch', fontsize=24)
...
def plot_roc_curve(roc_data, num_classes, save_path):
    ...
    plt.title('ROC Curves - AttBiLSTM with Lasso FS', fontsize=28, pad=20)
...
def save_model_architecture(model, file_path):
    ...
    with open(file_path, 'w') as f:
        f.write("Advanced AttBiLSTM Model Architecture with Lasso Feature Selection\n")
        f.write("=" * 70 + "\n\n")
        ...
        f.write(f"Feature Selection Method: Lasso\n\n")
...
def run_complete_analysis():
    """Run the complete AttBiLSTM analysis pipeline with Lasso feature selection"""

    print("=" * 60)
    print("Starting Advanced AttBiLSTM Analysis Pipeline with Lasso FS")
    print("=" * 60)
    ...
    print(f"Input features: {num_features} (selected by Lasso from {len(feature_names)} original features)")
    ...
    # Save model
    model_save_path = os.path.join(BASE_DIR, 'models', 'attbilstm_lasso_model.pth')
    torch.save({
    ...
    detailed_metrics['Feature Selection Method'] = 'Lasso'
    ...
    selected_features_df = pd.DataFrame({
        ...
        'lasso_importance': feature_importance_scores[selected_indices]
    })
    ...
    with open(summary_path, 'w') as f:
        f.write("Advanced AttBiLSTM Analysis Summary with Lasso Feature Selection\n")
        f.write("=" * 65 + "\n\n")
        ...
        f.write(f"Method: Lasso (L1 Regularization)\n")
        f.write(f"Base model: Logistic Regression with L1 penalty\n")
    ...
    print("\nFiles generated:")
    print("- Models: attbilstm_lasso_model.pth, model_architecture.txt")
    print("- Feature Selection: lasso_results.json, selected_features.csv, lasso_feature_importance.png")
