...
# Base directory
BASE_DIR = "AttBiLSTM_Analysis_With_MiG_FS"
...
# Initial split for MiG feature selection
...
def perform_mig_feature_selection(X_train, y_train, X_val, y_val,
    ...
    plot_mig_feature_importance(feature_importance, feature_names, selected_indices, n_features)

    print(f"MiG analysis results saved to: {mig_results_path}")
    print(f"MiG selector saved to: {mig_selector_path}")

    return selected_indices, selected_features, feature_importance



def plot_mig_feature_importance(feature_importance, feature_names, selected_indices, n_features):
    """Plot Mutual Information Gain feature importance"""
    ...
    # Create horizontal bar plot with color coding based on MI scores
    y_pos = np.arange(len(top_names))
    colors = []
    for mi_score in top_importance:
        if mi_score > 0.1:
    ...
    plt.title(f'Top {len(top_names)} Features by Mutual Information Gain', fontsize=16, pad=20)
    plt.grid(axis='x', alpha=0.3)

    # Add legend for color coding
    from matplotlib.patches import Patch
    legend_elements = [
        Patch(facecolor='darkgreen', label='MI > 0.1'),
        Patch(facecolor='green', label='0.05 < MI ≤ 0.1'),
        Patch(facecolor='orange', label='0.01 < MI ≤ 0.05'),
        Patch(facecolor='red', label='MI ≤ 0.01')
    ]
    ...
    # Save plot
    importance_plot_path = os.path.join(BASE_DIR, 'feature_selection', 'mig_feature_importance.png')
    plt.savefig(importance_plot_path, dpi=300, bbox_inches='tight')
    plt.savefig(importance_plot_path.replace('.png', '.pdf'), dpi=300, bbox_inches='tight')
    plt.close()

    # Plot distribution of all MI scores
    plt.figure(figsize=(15, 5))

    # Subplot 1: MI score distribution (linear scale)
    plt.subplot(1, 3, 1)
    ...
    plt.title('Distribution of MI Scores (Linear)', fontsize=14)
    plt.grid(alpha=0.3)

    # Subplot 2: MI score distribution (log scale)
    plt.subplot(1, 3, 2)
    # Add small constant to avoid log(0)
    mi_scores_log = np.log10(feature_importance + 1e-8)
    plt.hist(mi_scores_log, bins=50, color='lightcoral', edgecolor='darkred', alpha=0.7)
    plt.xlabel('log10(MI Score + ε)', fontsize=12)
    plt.ylabel('Number of Features', fontsize=12)
    plt.title('Distribution of log(MI Scores)', fontsize=14)
    plt.grid(alpha=0.3)

    # Subplot 3: Box plot of MI scores for different ranges
    plt.subplot(1, 3, 3)
    selected_mi = feature_importance[selected_indices]
    non_selected_mi = np.delete(feature_importance, selected_indices)

    box_data = [selected_mi, non_selected_mi]
    box_labels = ['Selected', 'Not Selected']
    ...
    plt.title('MI Score Comparison', fontsize=14)
    plt.grid(alpha=0.3)

    plt.tight_layout()

    dist_plot_path = os.path.join(BASE_DIR, 'feature_selection', 'mig_distributions.png')
    plt.savefig(dist_plot_path, dpi=300, bbox_inches='tight')
    plt.savefig(dist_plot_path.replace('.png', '.pdf'), dpi=300, bbox_inches='tight')
    plt.close()

    print(f"MiG importance plots saved to: {importance_plot_path}")
    print(f"MiG distribution plots saved to: {dist_plot_path}")



# Perform MiG Feature Selection
print("\n" + "=" * 60)
print("STARTING MUTUAL INFORMATION GAIN (MiG) FEATURE SELECTION")
print("=" * 60)

selected_indices, selected_features, feature_importance_scores = perform_mig_feature_selection(
    X_train_initial, y_train_initial, X_val_initial, y_val_initial,
    feature_names, n_features=2000
)

# Apply feature selection to all datasets
print("\nApplying MiG feature selection to datasets...")
...
def plot_confusion_matrix(cm, class_names, save_path):
    ...
    plt.title('Confusion Matrix of AttBiLSTM with Feature Selection',
...
def plot_roc_curve(roc_data, num_classes, save_path):
    ...
    plt.title('ROC Curves - AttBiLSTM with Feature Selection', fontsize=28, pad=20, weight='bold')
...
def save_model_architecture(model, file_path):
    ...
    with open(file_path, 'w') as f:
        f.write("Advanced AttBiLSTM Model Architecture with Feature Selection\n")
        f.write("=" * 80 + "\n\n")
        ...
        f.write("Feature Selection Method: [Method Name]\n\n")
...
def run_complete_analysis():
    """
    Run the complete AttBiLSTM analysis pipeline with feature selection
    ...
    print("=" * 80)
    print("STARTING COMPLETE AttBiLSTM ANALYSIS PIPELINE")
    print("=" * 80)
    ...
    def create_data_loaders_internal(X_train, y_train, X_val, y_val, batch_size=32):
        X_train_tensor = torch.FloatTensor(X_train)
        y_train_tensor = torch.LongTensor(y_train)
        X_val_tensor = torch.FloatTensor(X_val)
        y_val_tensor = torch.LongTensor(y_val)
    ...
    train_loader, val_loader = create_data_loaders_internal(
    ...
    # Import model class (assuming it's defined elsewhere)
    if 'AdvancedAttBiLSTM' not in globals():
        raise ValueError("AdvancedAttBiLSTM class not found. Please define the model first.")
    ...
    if 'train_model' not in globals():
        raise ValueError("train_model function not found. Please define it first.")
    ...
    torch.save({
        ...
        'feature_importance_scores': feature_importance_scores.tolist() if 'feature_importance_scores' in globals() else None,
    ...
    detailed_metrics['Feature Selection Method'] = 'Feature Selection Applied'
    ...
    if 'selected_features' in globals() and 'selected_indices' in globals():
        selected_features_df = pd.DataFrame({
            ...
            'importance_score': feature_importance_scores[
                selected_indices] if 'feature_importance_scores' in globals() else [1.0] * len(selected_indices)
        })
    ...
    with open(summary_path, 'w') as f:
        f.write("COMPREHENSIVE AttBiLSTM ANALYSIS SUMMARY\n")
        f.write("=" * 80 + "\n\n")
